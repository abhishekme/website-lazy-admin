...
import { Component, OnInit, AfterViewInit, ChangeDetectorRef,ElementRef, ViewChild, Output } from '@angular/core';
...
// ----------------------------------- Functionality -----------------------------------------
  ...
  loadState(event){
    console.log(event);
    if(typeof event === 'object' && event.countryId > 0){
      this.userModel.countryId  = event.countryId;
        //load States
        this.subscriptions.push(
           this.objUserServ.adminGetStateByCountry(event.countryId).subscribe(
            data => {
                this.stateList  = data.userData;
            })
        );        
    }else{
      this.stateList = [];
  ...
  loadCity(event){
    ...
    if(typeof event === 'object'){
      ...
      this.userModel.stateId  = stateId;
    }
    console.log(stateId);
  }
...
ngOnInit() {
    ...
    //load country list record
    this.objUserServ.adminGetCountryList().subscribe(
      data => {
        console.log(data);
          this.countryList  = (data.record);
