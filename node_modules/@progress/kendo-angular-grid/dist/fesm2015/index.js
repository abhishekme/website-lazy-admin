 ...
 * @hidden
 */
const observe = (list) => merge$1(of$1(list), list.changes);
/**
 * @hidden
 */
 ...
 * @hidden
 */
const getIterator = (data, { footers, level, dataIndex, parentGroupIndex, groupIndex }) => {
    const first = data[0];
    if (isPresent(first) && isGroupItem(first)) {
...
class Iterator {
    constructor(arr, dataIndex = 0, resultMap = (x) => x) {
        ...
        const iter = arr[iterator$1];
        this._innerIterator = iter ? arr[iterator$1]() : new ArrayIterator(arr);
    }
    [iterator$1]() {
        return this;
    }
...
class DataCollection {
    ...
    [iterator$1]() {
        return this.accessor()[iterator$1]();
    }
...
/** @nocollapse */
DomEventsService.ctorParameters = () => [];
...
class EditService {
    constructor(ngZone) {
        ...
        this.changed = this.changedSource.asObservable().pipe(switchMap$1(() => this.ngZone.onStable.asObservable().pipe(take$1(1))));
    }
...
ColumnList.empty = () => new ColumnList(new QueryList());
...
const syncRowsHeight = (table1, table2) => {
    ...
    const rows = zip$2(table1.rows, table2.rows);
    clearHeight(rows);
...
class ScrollSyncService {
    constructor(ngZone) {
        ...
        this.changes = new Subject$1();
        this.elements = [];
        this.changeSubscription = new Subscription$1();
        this.headerSubscription = new Subscription$1();
        this.bodySubscription = new Subscription$1();
        this.changeSubscription.add(this.changes.pipe(distinctUntilChanged$1((x, y) => x.scrollLeft === y.scrollLeft))
    ...
    registerEmitter(el, sourceType) {
        ...
        if (sourceType === "body" || sourceType === "header") {
            this.ngZone.runOutsideAngular(() => {
                const obs = fromEvent$1(el, "scroll").pipe(map$1(({ target: { scrollLeft, scrollRight } }) => ({
                    scrollLeft,
                    scrollRight,
                    sourceType
                })));
                const subscription = obs.pipe(filter$1(x => !this.source || this.source === x.sourceType), tap$1(x => this.source = x.sourceType))
                    .subscribe((x) => this.changes.next(x));
                subscription.add(obs.pipe(filter$1(x => this.source && this.source !== x.sourceType))
...
class ResizeService {
    constructor() {
        this.resizeSubscription = new Subscription$1(() => { });
...
/** @nocollapse */
ColumnReorderService.ctorParameters = () => [];
...
/** @nocollapse */
ScrollRequestService.ctorParameters = () => [];
...
class NavigationService {
    ...
    init(meta) {
        this.alive = true;
        this.metadata = meta;
        const onStableSubscriber = (...operators) => () => this.zone.isStable ?
            from$1([true]) :
            this.zone.onStable.pipe(take$1(1), ...operators);
        ...
        this.subs.add(this.domEvents.focus.pipe(switchMap$1(onStable, args => args))
            .subscribe(args => this.navigateTo(args.target)));
        this.subs.add(this.domEvents.focusOut.pipe(filter$1(() => this.mode !== 0 /* Standby */), switchMap$1(onStableSubscriber(takeUntil$1(this.domEvents.focus)), args => args))
            .subscribe(args => this.onFocusOut(args)));
        this.subs.add(this.domEvents.windowBlur.pipe(filter$1(() => this.mode !== 0 /* Standby */))
            .subscribe(() => this.onWindowBlur()));
        this.subs.add(
        // Closing the editor will not always trigger focusout in Firefox.
        // To get around this, we ensure that the cell is closed after editing.
        this.editService.changes.pipe(filter$1(e => e.action !== 'edit' && this.mode === 2 /* Content */), filter$1((e) => e.action === 'cellClose' && !e.prevented), switchMap$1(onStable, args => args))
        ...
        this.subs.add(this.domEvents.keydown.pipe(filter$1(args => args.keyCode === 9 /* tab */ && this.mode === 2 /* Content */), switchMapTo$1(this.domEvents.focusOut.pipe(takeUntil$1(
    ...
    onCellKeydown(args) {
        if (this.editService.isEditingCell()) {
            const confirm = args.keyCode === 13;
            const cancel = args.keyCode === 27;
...
/** @nocollapse */
ColumnInfoService.ctorParameters = () => [];
...
const validateColumnsField = (columns) => expandColumns(columns.toArray())
    ...
    .forEach(({ field }) => console.warn(`
            ...
            `));
const handleExpandCollapseService = (service, expandEmitter, collapseEmitter, map$$1) => (service.changes.pipe(filter$1(({ dataItem }) => isPresent(dataItem)))
...
class GridComponent {
    constructor(supportService, selectionService, wrapper, groupInfoService, groupsService, changeNotification, detailsService, editService, filterService, pdfService, responsiveService, renderer, excelService, ngZone, scrollSyncService, domEvents, columnResizingService, changeDetectorRef, columnReorderService, columnInfoService, navigationService, sortService, localization) {
        ...
        this.columnResizingSubscription = this.columnResizingService
            .changes
            .pipe(tap$1(e => this.columnResizingInProgress = e.type !== 'end'), filter$1(e => e.type === 'end'))
    ...
    ngOnChanges(changes) {
        ...
        if (isChanged("scrollable", changes) && this.isScrollable) {
            this.ngZone.onStable.pipe(take$1(1)).subscribe(() => this.attachScrollSync());
        }
    }
    ngAfterViewInit() {
    ...
    focusEditElement(containerSelector) {
        this.ngZone.runOutsideAngular(() => {
            this.ngZone.onStable.asObservable().pipe(take$1(1)).subscribe(() => {
    ...
    attachStateChangesEmitter() {
        this.stateChangeSubscription =
            merge$1(this.pageChange.pipe(map$1(x => ({
                filter: this.filter, group: this.group, skip: x.skip, sort: this.sort, take: x.take
            }))), this.sortChange.pipe(map$1(sort => ({ filter: this.filter, group: this.group, skip: this.skip, sort: sort, take: this.pageSize }))), this.groupChange.pipe(map$1(group => ({
                filter: this.filter, group: group, skip: this.skip, sort: this.sort, take: this.pageSize
            }))), this.filterChange.pipe(map$1(filter$$1 => ({
    ...
    syncHeaderHeight(observable) {
        return observable
            .pipe(filter$1(() => isPresent(this.lockedHeader)))
    ...
    columnsContainerChange() {
        this.columnsContainerChangeSubscription =
            this.syncHeaderHeight(this.columnsContainer.changes.pipe(filter$1(() => this.lockedColumns.length > 0), switchMap$1(() => this.ngZone.onStable.asObservable().pipe(take$1(1)))));
    }
    notifyResize(e) {
...
class ScrollerService {
    ...
    create(rowHeightService, skip, take$$1, total) {
        ...
        const subject = new BehaviorSubject$1(new ScrollAction(this.rowHeightService.offset(skip)));
        this.subscription = Observable$1.create(observer => {
            this.unsubscribe();
    ...
    onScroll({ scrollTop, offsetHeight, scrollHeight, clientHeight }, observer) {
        ...
        if (!up) {
            if (lastItemIndex >= this.lastLoaded && this.lastLoaded < this.total) {
                ...
                const atBottom = Math.round(scrollTop) === scrollHeight - clientHeight;
                if (atBottom) {
...
class ListComponent {
    ...
    attachContainerScroll() {
        ...
        this.ngZone.runOutsideAngular(() => this.subscriptions.add(fromEvent$1(this.container.nativeElement, 'scroll').pipe(map$1((event) => event.target), filter$1(() => !this.suspendService.scroll), tap$1(this.onContainerScroll.bind(this)), tap$1(() => this.resetNavigationViewport())).subscribe(this.dispatcher)));
        this.scrollSyncService.registerEmitter(this.container.nativeElement, "body");
        if (this.lockedContainer) {
            this.ngZone.runOutsideAngular(() => {
                this.subscriptions.add(merge$1(fromEvent$1(this.lockedContainer.nativeElement, 'mousewheel'), fromEvent$1(this.lockedContainer.nativeElement, 'DOMMouseScroll')).pipe(filter$1((event) => !event.ctrlKey), tap$1(preventLockedScroll(this.container.nativeElement)), map$1(wheelDeltaY))
                    .subscribe(x => this.container.nativeElement.scrollTop -= x));
                this.subscriptions.add(fromEvent$1(this.lockedContainer.nativeElement, 'keydown').pipe(filter$1((event) => event.keyCode === 34 /* page_down */ || event.keyCode === 33 /* page_up */)).subscribe((event) => {
    ...
    createScroller() {
        ...
        this.scrollerSubscription = observable.pipe(filter$1((x) => x instanceof PageAction), filter$1(() => {
            const temp = this.skipScroll;
            this.skipScroll = false;
            return !temp;
        }), tap$1(() => this.rebind = true))
            .subscribe((x) => this.ngZone.run(() => this.pageChange.emit(x)));
        this.scrollerSubscription.add(observable.pipe(filter$1((x) => x instanceof ScrollAction))
            .subscribe(this.scroll.bind(this)));
        this.scrollerSubscription.add(observable.pipe(filter$1((x) => x instanceof ScrollBottomAction))
    ...
    handleRowSync() {
        ...
        return merge$1(this.changeNotification.changes, this.groupsService.changes
            .pipe(filter$1(isLocked), switchMapTo$1(this.ngZone.onStable.asObservable().pipe(take$1(1)))), this.editService.changed, this.resizeService.changes).pipe(tap$1(() => this.resetNavigationViewport()), filter$1(isLocked))
            .subscribe(() => {
    ...
    handleRowNavigationLocked() {
        return this.navigationService.changes.pipe(filter$1(() => isPresent(this.lockedContainer)), delay$1(10))
            .subscribe((args) => {
    ...
    initResizeService() {
        this.resizeService.connect(merge$1(...this.resizeSensors.map(sensor => sensor.resize)));
    }
    syncContainerHeight() {
...
class BaseFilterCellComponent {
    ...
    ngAfterContentInit() {
        this.operationListSubscription = observe(this.operatorList)
            .pipe(map$1(q => q.toArray()), map$1(toJSON))
            .subscribe(x => {
            this.operators = x;
        });
    }
...
class FilterInputDirective {
    ...
    subscribeChanges() {
        this.changeRequestsSubscription = this.changeRequests
            .pipe(debounceTime$1(this.filterDelay))
...
class TableDirective {
    ...
    ngOnInit() {
        const obs = this.service
            .changes.pipe(filter$1(e => this.locked === e.locked));
        this.subscription = obs.pipe(filter$1(e => e.type === 'start'), tap$1(this.initState.bind(this)), map$1(columnsToResize), switchMap$1((take$$1) => obs.pipe(filter$1(e => e.type === 'resizeTable'), map$1(e => e.delta), bufferCount$1(take$$1)))).subscribe(this.resize.bind(this));
...
/** @nocollapse */
RowFilterModule.ctorParameters = () => [];
...
class DropTargetDirective {
    ...
    ngOnInit() {
        ...
        const changes = this.service.changes.pipe(filter$1(({ target }) => target === this));
        this.subscriptions.add(changes.pipe(filter$1(({ type }) => type === 'leave'))
            .subscribe(e => {
            this.leave.next(this.eventArgs(e));
        }));
        this.subscriptions.add(changes.pipe(filter$1(({ type }) => type === 'enter'))
            .subscribe(e => {
            this.enter.next(this.eventArgs(e));
        }));
        this.subscriptions.add(changes.pipe(filter$1(({ type }) => type === 'drop'))
            .subscribe(e => {
...
class GroupPanelComponent {
    ...
    attachTargets() {
        ...
        const enterStream = this.dropTargets
            .reduce((acc, target) => merge$1(acc, target.enter), from$1([]));
        const leaveStream = this.dropTargets
            .reduce((acc, target) => merge$1(acc, target.leave), from$1([]));
        const dropStream = this.dropTargets
            .reduce((acc, target) => merge$1(acc, target.drop), from$1([]));
        this.targetSubscription.add(enterStream.pipe(tap$1(_ => this.hint.removeLock()), filter$1(({ draggable, target }) => this.canDrop(draggable.context, target.context)), tap$1(this.enter.bind(this)), switchMapTo$1(dropStream.pipe(takeUntil$1(leaveStream.pipe(tap$1(this.leave.bind(this))))))).subscribe(this.drop.bind(this)));
    }
...
class DraggableColumnDirective {
    ...
    ngOnInit() {
        this.subscriptions.add(this.zone.runOutsideAngular(() => this.draggable.kendo.press.pipe(filter$1(_ => this.enabled), filter$1(({ originalEvent: { target } }) => target === this.element.nativeElement || skipButtons(target)), tap$1((e) => {
            const originalEvent = e.originalEvent;
            if (!e.isTouch) {
                originalEvent.preventDefault();
            }
            this.nav.navigateTo(originalEvent.target);
        }), switchMap$1(preventOnDblClick(this.draggable.kendo.release)), tap$1(down => {
            this.hint.create(down, this.element.nativeElement, this.context.hint);
            this.cue.create();
        }), switchMap$1(down => this.draggable.kendo.drag.pipe(tap$1((e) => {
            if (e.isTouch) {
                e.originalEvent.preventDefault();
            }
        }), tap$1(this.hint.attach()), tap$1(this.cue.attach()), takeUntil$1(this.draggable.kendo.release), map$1(move => ({ move, down })))), tap$1(this.performDrag.bind(this)), switchMapTo$1(this.draggable.kendo.release)).subscribe(this.drop.bind(this))));
    }
    ngOnDestroy() {
        if (this.subscriptions) {
            this.subscriptions.unsubscribe();
...
class HeaderComponent {
    ...
    attachTargets() {
        ...
        this.targetSubscription = new Subscription$1();
        const enterStream = merge$1(...this.dropTargets.map(target => target.enter));
        const leaveStream = merge$1(...this.dropTargets.map(target => target.leave));
        const dropStream = merge$1(...this.dropTargets.map(target => target.drop));
        this.targetSubscription.add(enterStream.pipe(tap$1(({ target, draggable }) => {
        ...
        }), filter$1(({ draggable, target }) => this.canDrop(draggable, target)), switchMap$1(this.trackMove.bind(this, leaveStream, dropStream)), map$1((e) => mergeObjects(e, { before: this.calculateBefore(e), changeContainer: e.changeContainer })), map$1(this.normalizeTarget.bind(this)), tap$1(this.enter.bind(this)), switchMapTo$1(dropStream.pipe(takeUntil$1(leaveStream.pipe(tap$1(this.leave.bind(this))))), (outerArgs, _) => outerArgs))
    ...
    trackMove(leaveStream, dropStream, e) {
        ...
        if (changed && isFirst) {
            return e.draggable.drag
                .pipe(takeUntil$1(leaveStream), takeUntil$1(dropStream), map$1(({ mouseEvent }) => mergeObjects({ changeContainer: true }, e, { mouseEvent })));
        }
 ...
 * @hidden
 */
const createMoveStream = (service, draggable) => mouseDown => draggable.kendo.drag.pipe(takeUntil$1(draggable.kendo.release.pipe(tap$1(() => service.end()))), map$1(({ pageX }) => ({
 ...
 * @hidden
 */
const preventOnDblClick$1 = release => mouseDown => of$1(mouseDown).pipe(delay$1(150), takeUntil$1(release));
/**
 * @hidden
 */
...
class ColumnHandleDirective {
    ...
    ngOnInit() {
        const service = this.service.changes.pipe(filter$1(() => this.column.resizable), filter$1(e => isPresent(e.columns.find(column => column === this.column))));
        this.subscriptions.add(service.pipe(filter$1(e => e.type === 'start'))
            .subscribe(this.initState.bind(this)));
        this.subscriptions.add(service.pipe(filter$1(e => e.type === 'resizeColumn'))
            .subscribe(this.resize.bind(this)));
        this.subscriptions.add(this.service.changes.pipe(filter$1(e => e.type === 'start'), filter$1(this.shouldUpdate.bind(this)), take$1(1) //on first resize only
        ).subscribe(this.initColumnWidth.bind(this)));
        this.subscriptions.add(this.zone.runOutsideAngular(() => this.draggable.kendo.press.pipe(tap$1(stopPropagation), tap$1(() => this.service.start(this.column)), switchMap$1(preventOnDblClick$1(this.draggable.kendo.release)), switchMap$1(createMoveStream(this.service, this.draggable)))
        ...
        this.subscriptions.add(service.pipe(filter$1(e => e.type === 'autoFitComplete'))
            .subscribe(this.sizeToFit.bind(this)));
        this.subscriptions.add(service.pipe(filter$1(e => e.type === 'triggerAutoFit'))
...
/** @nocollapse */
StringFilterMenuInputComponent.ctorParameters = () => [];
...
/** @nocollapse */
FilterMenuModule.ctorParameters = () => [];
...
/** @nocollapse */
ColumnMenuItemComponent.ctorParameters = () => [];
...
/** @nocollapse */
ColumnMenuService.ctorParameters = () => [];
...
/** @nocollapse */
ColumnMenuModule.ctorParameters = () => [];
 ...
 * data item ([more information and example]({% slug editing_reactive_forms_grid %}#toc-setup)).
 *
...
/** @nocollapse */
PDFMarginComponent$1.ctorParameters = () => [];
...
class PDFComponent extends PDFExportComponent {
    ...
    changePage(skip, _take, callback, columns) {
        this.ngZone.run(() => {
            this.pdfService.dataChanged.pipe(take$1(1)).subscribe(() => {
    ...
    onStable(callback) {
        ...
        setTimeout(() => {
            this.ngZone.onStable.asObservable().pipe(take$1(1)).subscribe(callback);
        }, 0); // tslint:disable-line: align
    }
}
